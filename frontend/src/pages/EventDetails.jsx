      ...
      if (status === "approved") {
        ...
        if (event.hostId) {
          await sendNotification({
            ...
            message: `${profile.displayName} just registered for ${event.title}!`,
            link: `/event/${id}`,
        ...
        if (event.hostId) {
          await sendNotification({
            ...
            message: `${profile.displayName} requested to join ${event.title}.`,
            link: `/event/${id}`,
            recipients: [event.hostId],
            type: "INFO"
          });
    ...
    try {
      ...
      if (studentIds.length > 0) {
        await sendNotification({
          ...
          message: `${event.title} has ended. Please share your feedback!`,
          link: `/event/${id}`,
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      ...
      if (event.hostId) {
        await sendNotification({
          title: "New Feedback",
          message: `${profile.displayName} left feedback for ${event.title}.`,
          link: `/event/${id}`,
          recipients: [event.hostId],
          type: "INFO"
        });
